...
class ProposerController extends Controller
{
    ...
    public function sendSearchAction(Request $request, $id){
        ...
        $userRepository = $this
            ...
            ->getRepository('AppBundle:User')
        ;
        $users = $userRepository->findBy(array('proposer' => $proposer));
    ...
    public function addCollaboratorAction(Request $request){
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                $userRegister = $this->get('app.user_register');
                $collaborator = $userRegister->addCollaborator($email, $proposer);
                ...
                $translated = $this->get('translator')->trans('proposer.addCollaborator.added');
                $session->getFlashBag()->add('info', $translated);

                return $this->redirectToRoute('edit_proposer', array('id' => $proposer->getId()));

            }
        }
        return $this->render('ProposerBundle:Proposer:addCollaborator.html.twig', [
            'form' => $form->createView(),
        ]);
    ...
    public function listCollaboratorAction(Request $request, $id){
        ...
        $proposer = $proposerRepository->findOneBy(array('id' => $id));
        ...
        $users = $repository->findBy(array('proposer' => $proposer, 'enabled' => 1));
        ...
        return $this->render('ProposerBundle:Proposer:collaboratorList.html.twig', array(
            'collaborators' => $users
        ));
    }
    ...
    public function archiveCollaboratorAction(Request $request, $id){
        ...
        $translated = $this->get('translator')->trans('proposer.addCollaborator.deleted');
        $session->getFlashBag()->add('info', $translated);

        return $this->redirectToRoute('list_collaborator', array('id' => $proposer->getId()));
    }
    ...
    public function changeAccessCollaboratorAction(Request $request, $id){
        ...
        $translated = $this->get('translator')->trans('proposer.addCollaborator.access.changed');
        $session->getFlashBag()->add('info', $translated);
